...
export default function SchemaManager() {
  ...
  const saveColumn = async () => {
    try {
      ...
      const columnConfig = {
        ...
        format: values.format, // Add format property here
        tableOptions: {
          width: values.width,
      ...
      if (oldColumnName && oldColumnName !== newColumnName) {
        ...
        if (updatedSchema.schema.required?.includes(oldColumnName)) {
          updatedSchema.schema.required = updatedSchema.schema.required.filter(
            (name: string) => name !== oldColumnName
          );
          if (values.required) {
  ...
  const getColumnsData = (schema: SchemaConfig) => {
    ...
    const finalOrder = [
      ...
      ...order.filter((key: string) => allKeys.includes(key)),
                      ...
                      {columnsData.map((item, index) => (
                        ...
                        >
                          {(
                            provided: {
                              innerRef:
                                | React.LegacyRef<HTMLTableRowElement>
                                | undefined;
