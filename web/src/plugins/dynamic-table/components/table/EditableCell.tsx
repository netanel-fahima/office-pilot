...
export const EditableCell: React.FC<EditableCellProps> = ({
  ...
  useEffect(() => {
    if (editing && property?.tableOptions?.componentType === "dropdown") {
      ...
      if (property.enum) {
        const enumOptions = property.enum.map((value: any) => ({
          label: String(value),
          value: value,
        }));
            ...
            const options = querySnapshot.docs.map((doc) => {
              ...
              return {
                label: data[labelField]
                  ...
                  : "ללא תווית",
                value:
                  data[valueField] !== undefined ? data[valueField] : doc.id,
              };
            });
          ...
          return (
            <Select style={{ width: "100%" }}>
              {dynamicOptions.map((option) => {
                const optionValue =
                  isNumericField && typeof option.value === "string"
                    ? Number(option.value)
                    : option.value;
    ...
    <td {...restProps} onDoubleClick={onDoubleClick}>
      {editing ? (
        <Form.Item
          ...
          valuePropName={
            property?.tableOptions?.componentType === "checkbox" ||
